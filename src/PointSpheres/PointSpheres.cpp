...
void PointSpheres::onCreate() {
    ...
    {
        pointSphereSystem.createPipelineLayout(defaultDescriptorLayout.descriptorSetLayout(), 0);
        pointSphereSystem.createPipeline(renderer.renderPass(), instanceShaderPaths, [this](vkb::GraphicsPipeline::PipelineConfigInfo& info) {
            info.inputAssembly.topology = VK_PRIMITIVE_TOPOLOGY_POINT_LIST;
            info.bindingDescription.clear();
            info.bindingDescription.push_back({0, sizeof(SphereData), VK_VERTEX_INPUT_RATE_VERTEX});
            info.attributeDescription.clear();
            info.attributeDescription.push_back({0, 0, VK_FORMAT_R32G32B32_SFLOAT, offsetof(SphereData, position)});
            info.attributeDescription.push_back({1, 0, VK_FORMAT_R32G32B32_SFLOAT, offsetof(SphereData, color)});
        });
    }
}

void PointSpheres::initializeObjects() {
...
void PointSpheres::createInstances() {
    vkDeviceWaitIdle(device.device());

    spheres.resize(INSTANCE_COUNT);
...
void PointSpheres::mainLoop(float deltaTime) {
