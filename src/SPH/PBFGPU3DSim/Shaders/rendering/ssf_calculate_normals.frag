...
float c_x = 2 * ubo.tanHalfFov / ubo.screenWidth;
float c_y = 2 * ubo.screenWidth * ubo.tanHalfFov / (ubo.screenHeight * ubo.screenHeight);
...
//    float dzdx = getDepth(x + dx, y) - z;
//    float dzdy = getDepth(x, y + dy) - z;
//    float dzdx2 = z - getDepth(x - dx, y);
//    float dzdy2 = z - getDepth(x, y - dy);
